...
# IMPORTANT: Debiased ERA5 data directory (source for SWAN extraction)
DEBIASED_ERA5_DIR = os.path.join(BASE_DIR, "data/era5/debiased")

# Directories for wind files (VARIABLE, one per hour) - OUTPUT from extraction
...
# Raw ERA5 directory (for statistics processing)
RAW_ERA5_DIR = os.path.join(BASE_DIR, "data/era5/raw")

# Create directories
os.makedirs(SWAN_BIN_DIR, exist_ok=True)
...
# ============================================================================
# Wind File Extraction for Nyenje Bay from DEBIASED ERA5 Data
# ============================================================================
...
def extract_swan_wind_from_debiased_era5(nc_file, output_dir, debias_factor=1.0):
    ...
    Since data is already debiased, debias_factor should be 1.0
    Creates one .wnd file per timestep for SWAN input
    """
    
    if not SCIPY_AVAILABLE:
        st.error("scipy not available. Run: pip install scipy")
        return 0, []
    ...
    try:
        ...
        st.error(f"Error extracting: {str(e)}")
        ds.close()
        return 0, []
...
# ============================================================================
# File Processing Functions for Statistics (from RAW ERA5 data)
# ============================================================================
...
def main():
    ...
    # ========================================================================
    # TAB 1: Data Acquisition & Processing
    # ========================================================================
    with tab1:
        ...
        with sub1:
            ...
            **After download:**
            ...
            2. Apply debiasing to create: `data/era5/debiased/`
            """)
...
c.retrieve(
    ...
    'era5_download.nc')
        ...
        # ================================================================
        # SUB-TAB 2: CCMP Download
        # ================================================================
        with sub2:
            st.markdown("### 🛰️ CCMP Satellite Data")
            st.info("CCMP wind data from NASA PODAAC for debiasing ERA5 data")
            
            st.markdown("""
            **Download Instructions:**
            1. Visit: https://podaac.jpl.nasa.gov/dataset/CCMP_WINDS_10M6HR_L4_V3_1
            2. Select area: 26°E to 30°E, 18°S to 10°S
            3. Select time period: 1971-2020
            4. Download NetCDF files
            
            **Why CCMP?**
            - CCMP provides high-quality satellite-based wind measurements
            - Used to debias ERA5 reanalysis data
            - Debiasing factor: 0.718 (ERA5 × 0.718 = CCMP equivalent)
            
            **Debiasing Process:**
            After downloading CCMP data, run the debiasing script to create:
            `data/era5/debiased/` directory with corrected wind files
            """)
        
        # ================================================================
        # SUB-TAB 3: Process Data (for Statistics from RAW ERA5)
        # ================================================================
        with sub3:
            st.markdown("### ⚙️ Process RAW ERA5 Data for Statistics")
            st.info("Process raw ERA5 files to generate wind statistics (wind roses, diurnal patterns, etc.)")
            
            data_dir = st.text_input(
                "RAW ERA5 Data Directory", 
                value=RAW_ERA5_DIR,
                key="process_dir",
                help="Directory containing raw ERA5 NetCDF files"
            )
            ...
            with col2:
                if st.button("📊 Process RAW ERA5 Files", key="process_real", use_container_width=True):
                    if not XARRAY_AVAILABLE:
        ...
        # ================================================================
        # SUB-TAB 4: SWAN Extract (from DEBIASED ERA5 Data)
        # ================================================================
        with sub4:
            st.markdown("###  Extract SWAN Wind Files from DEBIASED ERA5 Data")
            st.info("Extract wind data for Nyenje Bay 9×8 grid from DEBIASED ERA5 files")
            st.warning("⚠️ **Important:** These files should already be debiased (×0.718 applied)")
            st.caption("Creates ONE wind file per hour for VARIABLE SWAN input")
            
            # Source: Debiased ERA5 directory
            swan_data_dir = st.text_input(
                "DEBIASED ERA5 Directory", 
                value=DEBIASED_ERA5_DIR,
                key="swan_dir",
                help="Directory containing DEBIASED ERA5 NetCDF files (already multiplied by 0.718)"
            )
            
            # Output: Wind files for SWAN
            swan_output_dir = st.text_input(
                "Output Directory (SWAN Wind Files)", 
                value=WIND_FILES_DIR, 
                key="swan_out",
                help="Where the extracted .wnd files will be saved"
            )
            
            swan_year = st.selectbox("Select Year to Extract", list(range(1971, 2021)), index=49, key="swan_year")
            
            st.info(f"💡 **Note:** Since data is already debiased, the debiasing factor is set to 1.0")
            debias_factor = 1.0  # Fixed at 1.0 for debiased data
            
            # Show existing files
            existing = get_existing_wind_files(swan_output_dir)
            st.caption(f"📁 Existing SWAN wind files: {len(existing)} .wnd files")
            
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"**Source:** `{swan_data_dir}`")
            with col2:
                st.markdown(f"**Output:** `{swan_output_dir}`")
            
            if st.button("🚀 Extract SWAN Winds from Debiased ERA5", key="extract_swan", use_container_width=True, type="primary"):
                if not XARRAY_AVAILABLE:
                    st.error("Install: pip install xarray netCDF4 cfgrib")
                elif not SCIPY_AVAILABLE:
                    st.error("Install: pip install scipy")
                elif not os.path.exists(swan_data_dir):
                    st.error(f"❌ Debiased ERA5 directory not found: {swan_data_dir}")
                    st.info("Please ensure you have run the debiasing script first to create debiased files in `data/era5/debiased/`")
                else:
                    with st.spinner(f"Extracting wind files from DEBIASED data for {swan_year}..."):
                        os.makedirs(swan_output_dir, exist_ok=True)
                        
                        # Find debiased files for the selected year
                        year_files = glob.glob(os.path.join(swan_data_dir, f"*{swan_year}*.nc"))
                        year_files.extend(glob.glob(os.path.join(swan_data_dir, str(swan_year), "*.nc")))
                        year_files.extend(glob.glob(os.path.join(swan_data_dir, f"*{swan_year}*.nc4")))
                        
                        if not year_files:
                            st.warning(f"⚠️ No NetCDF files found for year {swan_year} in {swan_data_dir}")
                            st.info("Make sure debiased ERA5 files are in: `data/era5/debiased/`")
                        else:
                            st.info(f"Found {len(year_files)} debiased NetCDF file(s) for {swan_year}")
                            ...
                            for idx, nc_file in enumerate(year_files):
                                status_text.text(f"Processing: {os.path.basename(nc_file)} ({idx+1}/{len(year_files)})")
                                count, created = extract_swan_wind_from_debiased_era5(nc_file, swan_output_dir, debias_factor)
                                total_extracted += count
                                all_files.extend(created)
                                progress_bar.progress((idx + 1) / len(year_files))
                            ...
                            st.metric("Output Directory", swan_output_dir)
                            
                            st.session_state.extracted_wind_files = all_files
    ...
    with tab2:
        ...
        if not st.session_state.processed:
            st.info("⚠️ Please load data first (Data Acquisition & Process tab)")
            st.stop()
    ...
    # ========================================================================
    # TAB 4: SWAN Model (WITH ALL 5 SUB-TABS INCLUDING FIND HIGHEST WAVE)
    # ========================================================================
    with tab4:
        ...
        with swan_sub1:
            ...
            if REAL_BATHYMETRY is not None:
                ...
                st.caption("📌 This bathymetry is used for ALL SWAN runs")
            else:
            ...
            if available_years:
                ...
                if st.button("🌊 Run SWAN Analysis", type="primary"):
                    with st.spinner("Calculating wave distribution..."):
                        ...
                        # Apply debiasing if using raw data
                        if use_debiased and st.session_state.data_source != "Demo" and "Raw" in st.session_state.data_source:
        ...
        # ================================================================
        # SUB-TAB 5: Find Highest Wave (RESTORED)
        # ================================================================
        with swan_sub5:
            ...
            if st.button("🔍 Find Highest Wave Event", type="primary", key="find_highest"):
                with st.spinner("Searching through all timesteps (this may take a moment)..."):
                    df_sample = data['df'].copy()
                    wind_speeds = df_sample['speed'].values
                    
                    if use_debiased and st.session_state.data_source != "Demo" and "Raw" in st.session_state.data_source:
                        wind_speeds = wind_speeds * 0.718
                    
                    # Calculate wave heights for all timesteps
                    wave_heights, wave_periods = zip(*[calculate_wave_height(ws, fetch_km) for ws in wind_speeds])
                ...
                with col1:
                    ...
                    fig_hs.update_layout(
                        title=f"<b>Wave Height Distribution at Peak</b>",
                        xaxis_title="<b>West → East</b>",
                        yaxis_title="<b>North → South</b>",
                        height=400
                    )
                ...
                with col2:
                    ...
                    fig_tp.update_layout(
                        title="<b>Wave Period Distribution at Peak</b>",
                        xaxis_title="<b>West → East</b>",
                        yaxis_title="<b>North → South</b>",
                        height=400
                    )
