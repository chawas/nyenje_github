...
def main():
    ...
    with tab6:
        ...
        gallery_tab1, gallery_tab2, gallery_tab3 = st.tabs([
            "📸 Site Photos", "🗺️ Site Maps", "📊 Site Diagram"
        ])
        
        with gallery_tab1:
            st.subheader("Site Photographs")
            
            # Define image paths (adjust paths as needed)
            image_dir = os.path.join(BASE_DIR, "docs", "images", "nyenje")
            image_files = []
            ...
            if image_files:
                ...
                cols = st.columns(2)
                for idx, img_path in enumerate(image_files[:6]):  # Max 6 images
                    with cols[idx % 2]:
                        st.image(img_path, caption=os.path.basename(img_path), use_container_width=True)
                
                if len(image_files) > 6:
                    with st.expander(f"View {len(image_files)-6} more images"):
                        remaining_cols = st.columns(2)
                        for idx, img_path in enumerate(image_files[6:]):
                            with remaining_cols[idx % 2]:
                                st.image(img_path, caption=os.path.basename(img_path), use_container_width=True)
            else:
                st.info("📸 No site photos found. Add images to: `data/images/nyenje/`")
                
                # Display placeholder descriptions
                st.markdown("""
                ### Expected Images:
                - `nyenje_bay_overview.jpg` - Aerial/satellite view of the bay
                - `nyenje_shoreline.jpg` - Shoreline conditions
                - `nyenje_fetch.jpg` - Fetch length perspective
                - `nyenje_bathymetry.png` - Bathymetry visualization
                
                ### How to Add Images:
                1. Create directory: `mkdir -p data/images/nyenje`
                2. Add your images (JPG, PNG)
                3. Refresh the page
                """)
        
        with gallery_tab2:
            st.subheader("Site Maps & Satellite View")
            
            # Create a map using plotly
            import plotly.express as px
            
            # Define the Nyenje Bay bounding box
            nyenje_center = {'lat': -16.53, 'lon': 28.83}
            
            # Create a simple location map
            fig = go.Figure()
            
            # Add marker for Nyenje Bay
            fig.add_trace(go.Scattergeo(
                lon=[nyenje_center['lon']],
                lat=[nyenje_center['lat']],
                mode='markers+text',
            ...
            # Add Lake Kariba outline (approximate)
            lake_coords = {
                'lat': [-17.5, -17.5, -15.5, -15.5, -17.5],
                'lon': [27.5, 29.5, 29.5, 27.5, 27.5]
            }
            fig.add_trace(go.Scattergeo(
                lon=lake_coords['lon'],
                lat=lake_coords['lat'],
            ...
            fig.update_layout(
                title="Nyenje Bay Location - Lake Kariba",
                geo=dict(
                    scope='africa',
                    projection_type='mercator',
                    center=dict(lat=-16.5, lon=28.5),
                    lonaxis_range=[27.5, 29.5],
                    lataxis_range=[-18, -15],
                    showland=True,
                    landcolor='lightgray',
                    showocean=True,
            ...
            # Domain diagram
            st.subheader("Study Domain Visualization")
            
            # Create domain grid visualization
            x_coords = np.linspace(0, DOMAIN_X, NX)
            y_coords = np.linspace(0, DOMAIN_Y, NY)
            X, Y = np.meshgrid(x_coords, y_coords)
            ...
            # Add domain boundary
            domain_fig.add_trace(go.Scatter(
                x=[0, DOMAIN_X, DOMAIN_X, 0, 0],
                y=[0, 0, DOMAIN_Y, DOMAIN_Y, 0],
                mode='lines',
            ...
            domain_fig.add_trace(go.Scatter(
                ...
                name='Center Point (Analysis Location)'
            ))
            
            domain_fig.update_layout(
                title="Nyenje Bay Study Domain (4 km × 3.5 km, 9×8 grid)",
                xaxis_title="Distance from West (m)",
                yaxis_title="Distance from South (m)",
                height=500,
                showlegend=True
        ...
        with gallery_tab3:
            st.subheader("Site Diagram - Wind & Wave Setup")
            
            # Create a schematic diagram using plotly shapes
            schematic_fig = go.Figure()
            
            # Add fetch line (NE direction)
            schematic_fig.add_annotation(
                x=0.5, y=0.7,
                text="⬇️ Prevailing Wind (NE) ➡️",
                showarrow=False,
            ...
            # Add bay representation
            schematic_fig.add_shape(
                type="rect",
                x0=0.2, x1=0.8,
                y0=0.2, y1=0.6,
            ...
            schematic_fig.update_layout(
                title="Site Schematic - Wind Direction & Fetch",
                height=500,
            ...
            with st.expander("📋 Detailed Site Description", expanded=False):
                st.markdown("""
                ### Nyenje Bay Characteristics
                
                **Physical Setting:**
                - Located on the southern shore of Lake Kariba
                - Fetch-limited bay with 4 km effective fetch length
                - Maximum depth: ~30 m, Minimum depth: ~1.5 m
                - Sheltered from strong southerly and westerly winds
                
                **Wind Regime:**
                - Prevailing direction: Northeast (NE)
                - Mean wind speed: ~3-4 m/s
                - Extreme gusts: Up to 20 m/s
                
                **Wave Climate:**
                - Fetch-limited wave growth
                - Maximum significant wave height: ~0.8-1.2 m
                - Peak wave periods: 2-4 seconds
                
                **Study Purpose:**
                - Floating Photovoltaic (FPV) structural design
                - 50-year extreme value analysis
                - CCMP-debiased ERA5 wind data (×0.718 factor)
                """)  
