...
# SWAN Base Directory
SWAN_BASE_DIR = "/home/chawas/deployed/nyenje_github/swan"
SWAN_BIN_DIR = os.path.join(SWAN_BASE_DIR, "bin")
SWAN_EXE = os.path.join(SWAN_BIN_DIR, "swan.exe")
SWAN_INPUT_FILE = os.path.join(SWAN_BASE_DIR, "INPUT")  # SWAN reads this file

# Directories for wind files (VARIABLE, one per hour)
WIND_FILES_DIR = "/home/chawas/deployed/nyenje_github/data/wind_files/nyenje_era5_debiased"

# Output directories
SWAN_OUTPUTS_DIR = "/home/chawas/deployed/nyenje_github/swan/outputs"
SWAN_INPUTS_ARCHIVE = "/home/chawas/deployed/nyenje_github/swan/inputs_archive"
...
# ============================================================================
# Load CONSTANT Bathymetry (same for all runs)
# ============================================================================
...
def generate_swan_wave_distribution(wind_speed, wind_direction):
    ...
    Uses CONSTANT bathymetry (REAL_BATHYMETRY)
    """
    if REAL_BATHYMETRY is not None:
        depth = REAL_BATHYMETRY
    else:
        # Fallback to synthetic if real not available
...
# ============================================================================
# SWAN Input File Generation (CONSTANT bathymetry, VARIABLE wind)
# ============================================================================

def create_swan_input_file(wind_file_path, output_file_path, timestamp):
    """
    Create SWAN INPUT file with:
    - CONSTANT bathymetry (nyenje_bathymetry_9x8.bot)
    - VARIABLE wind file (specific to this timestamp)
    """
    
    swan_content = f"""PROJECT 'ERA5' '{timestamp[-4:]}' 'ERA5 run' ' ' ' '
MODE STATIONARY TWODIMENSIONAL
COORDINATES CARTESIAN
CGRID REGULAR 0.0 0.0 0.0 4000.0 3500.0 8 7 CIRCLE 36 0.04 1.00 25
! CONSTANT BATHYMETRY - same for all runs
INPGRID BOTTOM REGULAR 0.0 0.0 0.0 8 7 500.0 500.0
READINP BOTTOM 1.0 '{BATHYMETRY_FILE}' 1 0 FREE
! VARIABLE WIND - changes for each timestamp
INPGRID WIND REGULAR 0.0 0.0 0.0 8 7 500.0 500.0
READINP WIND 1 '{wind_file_path}' 4 0 FREE
...
def run_swan_for_timestamp(timestamp, wind_file_path, output_file_path):
    """Run SWAN for a specific timestamp"""
    
    # Create SWAN INPUT file
    swan_content = create_swan_input_file(wind_file_path, output_file_path, timestamp)
    
    # Write to SWAN's default INPUT file
    with open(SWAN_INPUT_FILE, 'w') as f:
        f.write(swan_content)
    
    # Also archive the input file for records
    archive_file = os.path.join(SWAN_INPUTS_ARCHIVE, f"swan_input_{timestamp}.swn")
...
# ============================================================================
# Extract Wind Data from ERA5 (VARIABLE, one file per hour)
# ============================================================================

def extract_swan_wind_from_era5(nc_file, output_dir, debias_factor=0.718):
    """Extract wind for Nyenje Bay 9x8 grid from ERA5 file (VARIABLE wind)"""
    ...
    try:
        ...
        u_interp = u10.interp(latitude=grid['lat'][:,0], longitude=grid['lon'][0,:], method='linear')
        v_interp = v10.interp(latitude=grid['lat'][:,0], longitude=grid['lon'][0,:], method='linear')
        ...
        for t_idx in range(n_times):
            u_values = u_interp.isel(time=t_idx).values * debias_factor
            v_values = v_interp.isel(time=t_idx).values * debias_factor
                ...
                with open(wind_file, 'w') as f:
                    for iy in range(grid['ny']):
                        ...
                        for ix in range(grid['nx']):
                            row.append(u_values[iy, ix])
                            row.append(v_values[iy, ix])
        ...
        st.warning(f"Error extracting: {e}")
        ds.close()
        return 0
...
def main():
    ...
    with tab1:
        ...
        with sub4:
            st.markdown("### 🌊 Extract SWAN Wind Files (VARIABLE)")
            st.info("Extract wind data for Nyenje Bay 9×8 grid - creates ONE wind file per hour")
            st.caption("These wind files will be used as VARIABLE input for SWAN runs")
            
            swan_data_dir = st.text_input("ERA5 Directory", value="./data/era5/raw", key="swan_dir")
            swan_output_dir = st.text_input("Output Directory", value=WIND_FILES_DIR, key="swan_out")
            ...
            if st.button("🚀 Extract SWAN Winds", key="extract_swan"):
                with st.spinner(f"Extracting wind files for {swan_year}..."):
                    ...
                    # Find files for the selected year
                    ...
                    for nc_file in year_files:
                        count = extract_swan_wind_from_era5(nc_file, swan_output_dir, debias_factor)
                        total_extracted += count
                    
                    st.success(f"✅ Extracted {total_extracted} wind files to {swan_output_dir}")
                    st.info(f"Wind files are organized by year in subdirectories")
    ...
    with tab4:
        ...
        st.info("🔸 **VARIABLE:** Wind files (one per hour from ERA5 debiased data)")
        ...
        # ================================================================
        # Single Analysis with Bathymetry Map
        # ================================================================
        with swan_sub1:
            st.subheader("Single Time Step Wave Analysis")
            st.markdown("**CONSTANT Bathymetry** + **VARIABLE Wind** (select date/time below)")
            
            # Display REAL Bathymetry Map (CONSTANT)
            st.subheader("🗺️ Nyenje Bay Bathymetry (CONSTANT - same for all runs)")
            if REAL_BATHYMETRY is not None:
                fig_bathy = go.Figure(data=go.Heatmap(
                    z=REAL_BATHYMETRY, x=[f"{i*500}m" for i in range(9)], y=[f"{i*500}m" for i in range(8)],
                    colorscale='Blues', text=np.round(REAL_BATHYMETRY, 1),
                    texttemplate='%{text} m', textfont={"size": 10}, colorbar_title="Depth (m)"
                ))
                fig_bathy.update_layout(
                    title="Nyenje Bay Bathymetry (REAL Data - CONSTANT)",
                ...
                st.warning("Bathymetry file not found. Place nyenje_bathymetry_9x8.bot in data/bathymetry/")
            ...
            if not available_years:
                ...
                if st.button("🌊 Run SWAN Analysis", key="run_single"):
                    with st.spinner("Calculating wave distribution..."):
                        ...
                        # Generate wave distribution using CONSTANT bathymetry
...
#echo "✅ Complete era5_analysis.py with clear CONSTANT/VARIABLE distinction"