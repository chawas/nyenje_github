...
Complete wind, wave, and SWAN wave modeling analysis with gust calculations

Features:
- 10-min, 3-min wind averages
- 10-sec, 3-sec gust calculations
- JONSWAP fetch-limited wave predictions
- SWAN wave model integration (9x8 grid)
...
# ============================================================================
# Constants and Gust Factors
# ============================================================================

# Gust factors for Lake Kariba (over water conditions)
GUST_FACTORS = {
    '3sec': 1.45,   # 3-second gust factor (peak gust)
    '10sec': 1.35,  # 10-second gust factor
    '3min': 1.15,   # 3-minute average factor
    '10min': 1.00,  # 10-minute average (reference)
}
...
def calculate_wind_averages(wind_speed_series, time_axis):
    """
    Calculate 3-min, 10-min averages and 3-sec, 10-sec gusts from 10-minute mean winds
    """
    ws_series = pd.Series(wind_speed_series, index=time_axis)
    
    # Rolling averages (using 3 and 10 as proxy for minutes)
    ...
    # Gusts using empirical factors for over-water conditions
...
def process_demo_data():
    ...
    # Calculate wind averages for demo data
...
def main():
    ...
    with tab2:
        ...
        with sub4:
            ...
            | 3-sec gust | ×{GUST_FACTORS['3sec']} | Peak instantaneous gust |
            """)
            
            # Display wind parameters plot
            df = data['df']
            ...
            if not available_years:
                ...
                if st.button("🌊 Run SWAN Analysis", key="run_single"):
                    with st.spinner("Calculating wave distribution..."):
                        ...
                        fig.update_layout(title=f"Wave Height Distribution - {selected_year}-{selected_month:02d}-{selected_day:02d} {selected_hour:02d}:00<br>Wind: {wind_speed:.1f} m/s from {wind_direction:.0f}°", xaxis_title="West → East", yaxis_title="North → South", height=550)
                        st.plotly_chart(fig, use_container_width=True)
                        ...
                        with st.expander("View Bathymetry Profile"):
                            fig_bathy = go.Figure(data=go.Heatmap(z=bathymetry, x=[f"{i*500}m" for i in range(9)], y=[f"{i*500}m" for i in range(8)], colorscale='Blues', text=np.round(bathymetry, 1), texttemplate='%{text} m'))
                            fig_bathy.update_layout(title="Bathymetry (Depth in meters)", xaxis_title="West → East", yaxis_title="North → South", height=450)
