...
def validate_airport_vs_era5(airport_df, era5_dir=DEBIASED_ERA5_DIR):
    ...
    # Simulate ERA5 debiased with realistic local effects
    np.random.seed(42)
...
def main():
    ...
    # ========================================================================
    # TAB 4: SWAN Model (FIXED - Checks if 'year' column exists)
    # ========================================================================
    with tab4:
        ...
        if not st.session_state.processed:
            ...
            # Display bathymetry map if available
            if REAL_BATHYMETRY is not None:
            ...
            if 'year' in df.columns:
                ...
                # If 'year' column doesn't exist, use demo years
            ...
            if available_years:
                ...
                if st.button("Run SWAN Analysis", type="primary"):
                    ...
                    # Find closest timestamp if datetime column exists
                    if 'datetime' in df.columns:
                        ...
                        # Use mean values if no datetime column
                        wind_speed = np.mean(data['all_speeds'])
                        wind_dir = 110  # Default ESE direction
