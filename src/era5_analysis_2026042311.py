...
- Added DEBIASING and VALIDATION buttons
- Added DOMAIN EXTENT information
- Added Wind Averages Comparison Table
- Added progress bar for Process Data
- Added all 4 SWAN Model menus
...
NYENJE_WEST_LON = 28.81113    # Western boundary
NYENJE_EAST_LON = 28.84887    # Eastern boundary
NYENJE_SOUTH_LAT = -16.54577  # Southern boundary
NYENJE_NORTH_LAT = -16.51423  # Northern boundary

# Domain size in degrees
NYENJE_LON_RANGE = NYENJE_EAST_LON - NYENJE_WEST_LON   # 0.03774°
NYENJE_LAT_RANGE = NYENJE_NORTH_LAT - NYENJE_SOUTH_LAT # 0.03154°

# Grid parameters
NX = 9
NY = 8
DOMAIN_X = 4000.0  # Width in meters (West-East)
DOMAIN_Y = 3500.0  # Height in meters (South-North)
CELL_SIZE_X = DOMAIN_X / (NX - 1)  # 500 m
CELL_SIZE_Y = DOMAIN_Y / (NY - 1)  # 500 m

# Conversion factors (at 16.53°S)
METERS_PER_DEGREE_LAT = 111000.0   # 1° latitude ≈ 111 km
METERS_PER_DEGREE_LON = 106000.0  # 1° longitude ≈ 106 km at this latitude
...
def validate_debiased_data(era5_file, ccmp_file):
    ...
    try:
        ...
        # For CCMP, get mean if time dimension exists
...
def process_all_files(data_dir, target_lat=-16.53, target_lon=28.83, progress_callback=None):
    """Process all ERA5 files for wind statistics with progress bar"""
    all_files = []
...
def process_demo_data():
    ...
    # Wind directions with prevailing pattern from SE
...
def main():
    ...
    # Domain Extent Card - VISIBLE AT TOP
    st.markdown("""
    <div style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); padding: 15px; border-radius: 10px; color: white; margin: 10px 0;">
        <h3>📍 Nyenje Bay Domain Extent</h3>
        <table style="width: 100%; color: white;">
            <tr><td><b>Center Point:</b></td><td>16.53°S, 28.83°E</td><td><b>Domain Size:</b></td><td>4.0 km × 3.5 km</td></tr>
            <tr><td><b>West Boundary:</b></td><td>28.81113°E</td><td><b>East Boundary:</b></td><td>28.84887°E</td></tr>
            <tr><td><b>South Boundary:</b></td><td>16.54577°S</td><td><b>North Boundary:</b></td><td>16.51423°S</td></tr>
            <tr><td><b>Grid Resolution:</b></td><td>500 m × 500 m</td><td><b>Grid Size:</b></td><td>9 × 8 points</td></tr>
        </table>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.session_state.processed:
            ...
            # Show wind average comparison in sidebar
            df = data['df']
            ...
            st.caption(f"3-sec Gust: {df['gust_3sec'].mean():.2f} m/s")
            st.caption(f"10-sec Gust: {df['gust_10sec'].mean():.2f} m/s")
        else:
...
c.retrieve(
    ...
    'era5_download.nc')
        ...
        with sub2:
            ...
            st.info(f"Apply CCMP-based debiasing factor ({DEBIAS_FACTOR}) to ERA5 wind data")
            
            col1, col2 = st.columns(2)
            with col1:
                debias_start = st.selectbox("Start Year", list(range(1971, 2021)), index=0, key="debias_start")
                debias_end = st.selectbox("End Year", list(range(1971, 2021)), index=49, key="debias_end")
            with col2:
                st.markdown(f"**Input (RAW):** `{RAW_ERA5_DIR}`")
                st.markdown(f"**Output (Debiased):** `{DEBIASED_ERA5_DIR}`")
                raw_count = len(glob.glob(os.path.join(RAW_ERA5_DIR, "*.nc")))
                st.caption(f"Found {raw_count} raw files")
            
            years_to_process = list(range(debias_start, debias_end + 1))
        ...
        with sub3:
            ...
            with col2:
                ...
                st.caption("- Bias calculation (ERA5 - CCMP)")
                st.caption("- Improvement after debiasing")
        ...
        # SUB-TAB 4: Process Data (WITH PROGRESS BAR)
        with sub4:
            st.markdown("### ⚙️ Process ERA5 Data for Statistics")
            st.info("Process raw or debiased ERA5 files to generate wind statistics")
            
            data_dir = st.radio("Select data source:", ["Raw ERA5", "Debiased ERA5"], horizontal=True, key="data_source_radio")
            use_dir = RAW_ERA5_DIR if data_dir == "Raw ERA5" else DEBIASED_ERA5_DIR
            st.info(f"Using: {use_dir}")
    ...
    # ========================================================================
    # TAB 2: Wind Statistics (WITH WIND AVERAGES COMPARISON TABLE)
    # ========================================================================
    with tab2:
        ...
        wind_sub1, wind_sub2, wind_sub3, wind_sub4, wind_sub5 = st.tabs([
            "📈 Summary Statistics", "📊 Wind Averages Comparison", "🌅 Wind Rose", "⏱️ Time Series", "🔥 Extremes"
        ])
        ...
        with wind_sub1:
            ...
            with col1:
                ...
                st.metric("Std Deviation", f"{np.std(data['all_speeds']):.2f} m/s")
            with col2:
                st.metric("Max Wind Speed", f"{data['global_max']['speed']:.2f} m/s")
                st.metric("Year of Max", f"{data['global_max']['year']}")
                st.metric("99th Percentile", f"{np.percentile(data['all_speeds'], 99):.2f} m/s")
            with col3:
            ...
            fig = go.Figure()
            fig.add_trace(go.Histogram(x=data['all_speeds'], nbinsx=30, marker_color='steelblue', opacity=0.7))
            fig.update_layout(title="Wind Speed Histogram", xaxis_title="Wind Speed (m/s)", yaxis_title="Frequency", height=400)
            st.plotly_chart(fig, use_container_width=True)
        
        # WIND AVERAGES COMPARISON TABLE
        with wind_sub2:
            st.subheader("📊 Wind Averages Comparison")
            st.markdown("Comparison of different averaging periods and gust factors")
            ...
            comparison_data = {
                ...
                'Min (m/s)': [
                    df['wind_speed_hourly'].min(),
                    df['wind_speed_3min'].min(),
                    df['wind_speed_10min'].min(),
                    df['gust_3sec'].min(),
                    df['gust_10sec'].min(),
                ],
                'Std Dev (m/s)': [
                    df['wind_speed_hourly'].std(),
                    df['wind_speed_3min'].std(),
                    df['wind_speed_10min'].std(),
                    df['gust_3sec'].std(),
                    df['gust_10sec'].std()
                ],
                'Gust Factor': [
            ...
            st.dataframe(
                comparison_df.style.format({
                    ...
                    'Std Dev (m/s)': '{:.2f}',
                    'Gust Factor': '{:.2f}'
                }).background_gradient(cmap='Blues', subset=['Mean (m/s)']),
            ...
            fig.add_trace(go.Bar(x=comparison_df['Parameter'], y=comparison_df['Mean (m/s)'], 
                                name='Mean', marker_color='steelblue'))
            fig.add_trace(go.Bar(x=comparison_df['Parameter'], y=comparison_df['Max (m/s)'], 
                                name='Max', marker_color='coral'))
            fig.update_layout(title="Wind Speed Comparison by Averaging Period", 
                            xaxis_title="Parameter", yaxis_title="Wind Speed (m/s)",
                            barmode='group', height=500)
            st.plotly_chart(fig, use_container_width=True)
            
            # Download button
            csv = comparison_df.to_csv(index=False)
            st.download_button("📥 Download Averages Comparison", csv, "wind_averages_comparison.csv", "text/csv")
        ...
        with wind_sub5:
            ...
            if 'yearly_max' in data and data['yearly_max']:
                ...
                fig.update_layout(title="Annual Maximum Wind Speeds", xaxis_title="Year", 
                ...
                if SCIPY_AVAILABLE and len(values) >= 5:
                    ...
                    return_df = pd.DataFrame([{'Return Period (years)': rp, 'Wind Speed (m/s)': round(return_speeds[rp], 2)} 
                                             for rp in return_periods])
    ...
    with tab3:
        ...
        with col3:
            ...
            st.metric("Sig Period (T₁/₃)", f"{np.percentile(wave_periods, 66.7):.2f} s")
        with col4:
            st.metric("10% Highest Waves", f"{np.percentile(wave_heights, 90):.3f} m")
            st.metric("1% Highest Waves", f"{np.percentile(wave_heights, 99):.3f} m")
    ...
    # ========================================================================
    # TAB 4: SWAN Model (WITH ALL 4 MENUS)
    # ========================================================================
    with tab4:
        ...
        # Create 4 sub-tabs for SWAN Model
        ...
        with swan_sub2:
            ...
            if st.button("Calculate Monthly Maxima", type="primary", use_container_width=True):
                with st.spinner("Calculating monthly maxima..."):
                    ...
                    # Time series plot
                    fig1 = go.Figure()
                    fig1.add_trace(go.Scatter(x=monthly_max['year_month'], y=monthly_max['wave_height'], 
                                             mode='lines+markers', name='Monthly Max Hₛ',
                                             line=dict(color='blue', width=1), marker=dict(size=3)))
                    fig1.update_layout(title="Monthly Maximum Wave Heights Over Time", 
                                      xaxis_title="Year-Month", yaxis_title="Wave Height (m)", 
                    ...
                    fig2 = go.Figure()
                    fig2.add_trace(go.Bar(x=monthly_stats['month_name'], y=monthly_stats['max'], 
                                         name='Maximum', marker_color='red'))
                    fig2.add_trace(go.Bar(x=monthly_stats['month_name'], y=monthly_stats['mean'], 
                                         name='Mean', marker_color='blue'))
                    fig2.update_layout(title="Monthly Wave Height Statistics", 
                                      xaxis_title="Month", yaxis_title="Wave Height (m)", 
        ...
        with swan_sub3:
            ...
            if st.button("Calculate Yearly Maxima", type="primary", use_container_width=True):
                with st.spinner("Calculating yearly maxima..."):
                    ...
                    fig.update_layout(title="Yearly Maximum Wave Heights", 
                                     xaxis_title="Year", yaxis_title="Wave Height (m)", height=500)
                    st.plotly_chart(fig, use_container_width=True)
        ...
        with swan_sub4:
            ...
            if st.button("Run Extreme Value Analysis", type="primary", use_container_width=True):
                if not SCIPY_AVAILABLE:
                    ...
                    with st.spinner("Performing extreme value analysis..."):
                        ...
                        if return_heights:
                            ...
                            return_df = pd.DataFrame([{'Return Period (years)': rp, 'Wave Height (m)': round(h, 3)} 
                                                     for rp, h in return_heights.items()])
                            st.dataframe(return_df, use_container_width=True)
                            
                            # Gumbel fit plot
                            fig = go.Figure()
                            fig.add_trace(go.Scatter(x=list(return_heights.keys()), y=list(return_heights.values()), 
                                                    mode='lines+markers', name='Gumbel Fit',
                                                    line=dict(color='red', width=2), marker=dict(size=10, color='red')))
                            fig.update_layout(title="Return Period Wave Heights", 
                                            xaxis_title="Return Period (years)", yaxis_title="Wave Height (m)", 
                            ...
                            # Download results
                            results = {'return_periods': return_heights, 'historical_max': max_event, 
                                      'gumbel_params': {'loc': params[0], 'scale': params[1]}}
                            st.download_button("📥 Download Extreme Value Results", 
                                             json.dumps(results, indent=2), 
                                             "extreme_value_results.json", "application/json")
                        else:
