#cat > /home/chawas/deployed/nyenje_github/src/era5_analysis.py << 'EOF'
#!/usr/bin/env python3
"""
    ...
    try:
        ...
        for line in lines:
            if line.strip() and not line.startswith('!'):
                ...
                if len(values) == NX:  # Should be 9 columns
                    depth_grid.append(values)
        
        if len(depth_grid) == NY:  # Should be 8 rows
            return np.array(depth_grid)
        else:
            st.warning(f"Bathymetry file has {len(depth_grid)} rows, expected {NY}")
            return None
    except Exception as e:
        st.error(f"Error loading bathymetry: {e}")
        return None

# Load the real bathymetry at startup
REAL_BATHYMETRY = load_real_bathymetry()
if REAL_BATHYMETRY is not None:
    st.sidebar.success("✅ Loaded REAL Nyenje Bay bathymetry")
else:
    st.sidebar.error("⚠️ Using synthetic bathymetry - REAL file not found")
...
def calculate_wind_averages(wind_speed_series, time_axis):
    """Calculate 3-min, 10-min averages and 3-sec, 10-sec gusts"""
...
def create_wind_parameters_plot(wind_df):
    """Create 6-panel wind parameters plot including gusts"""
...
def calculate_wave_height(wind_speed, fetch_km=50):
    """Calculate significant wave height and period using JONSWAP fetch-limited formula"""
    g = 9.81
...
def generate_swan_wave_distribution(wind_speed, wind_direction):
    ...
    Uses the REAL bathymetry file
    """
    # Use the real bathymetry
    if REAL_BATHYMETRY is not None:
    ...
    # Create grid coordinates for fetch calculation
    x = np.linspace(0, DOMAIN_X, NX)
    y = np.linspace(0, DOMAIN_Y, NY)
    X, Y = np.meshgrid(x, y)
    
    # Fetch length based on wind direction
    ...
    # Period is also depth-limited in very shallow water
...
def process_all_files(data_dir, target_lat=-16.53, target_lon=28.83, progress_callback=None):
    ...
    # Calculate wind averages and gusts
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("Lake Kariba is landlocked and protected by surrounding terrain.")
        ...
        fetch_km = st.slider("Fetch Length (km)", 1, 20, 4, 
                              help="Nyenje Bay fetch is approximately 4 km")
        use_debiased = st.checkbox("Apply CCMP Debiasing (×0.718)", value=True,
                                    help="Corrects ERA5 overestimation using satellite data")
        ...
        with swan_sub1:
            ...
            if not available_years:
                ...
                if st.button("🌊 Run SWAN Analysis", key="run_single"):
                    with st.spinner("Calculating wave distribution using REAL bathymetry..."):
                        ...
                        # Generate wave distribution using REAL bathymetry
                        wave_heights, wave_periods, bathymetry = generate_swan_wave_distribution(wind_speed, wind_direction)
                        
                        # Wave Height Heatmap
                        fig_hs = go.Figure(data=go.Heatmap(
                            z=wave_heights * 100, x=[f"{i*500}m" for i in range(9)], y=[f"{i*500}m" for i in range(8)],
                            colorscale='Viridis', text=np.round(wave_heights * 100, 1),
                            texttemplate='%{text} cm', textfont={"size": 10}, colorbar_title="Wave Height (cm)"
                        ))
                        fig_hs.update_layout(title=f"Wave Height Distribution - {selected_year}-{selected_month:02d}-{selected_day:02d} {selected_hour:02d}:00<br>Wind: {wind_speed:.1f} m/s from {wind_direction:.0f}°", xaxis_title="West → East", yaxis_title="North → South", height=500)
                        st.plotly_chart(fig_hs, use_container_width=True)
                        
                        # Wave Period Heatmap
                        fig_tp = go.Figure(data=go.Heatmap(
                            z=wave_periods, x=[f"{i*500}m" for i in range(9)], y=[f"{i*500}m" for i in range(8)],
                            colorscale='Plasma', text=np.round(wave_periods, 1),
                            texttemplate='%{text} s', textfont={"size": 10}, colorbar_title="Wave Period (s)"
                        ))
                        fig_tp.update_layout(title=f"Wave Period Distribution - {selected_year}-{selected_month:02d}-{selected_day:02d} {selected_hour:02d}:00", xaxis_title="West → East", yaxis_title="North → South", height=500)
                        ...
                        # Show REAL Bathymetry Profile
                        with st.expander("View REAL Bathymetry Profile"):
                            fig_bathy = go.Figure(data=go.Heatmap(
                                z=bathymetry, x=[f"{i*500}m" for i in range(9)], y=[f"{i*500}m" for i in range(8)],
                                colorscale='Blues', text=np.round(bathymetry, 1),
                                texttemplate='%{text} m', textfont={"size": 10}, colorbar_title="Depth (m)"
                            ))
                            fig_bathy.update_layout(title="Nyenje Bay Bathymetry (REAL Data)", xaxis_title="West → East", yaxis_title="North → South", height=450)
                            st.plotly_chart(fig_bathy, use_container_width=True)
                            
                            # Show depth statistics
                            st.markdown(f"""
                            | Depth Statistics | Value |
                            |-----------------|-------|
                            | **Minimum Depth** | {bathymetry.min():.1f} m |
                            | **Maximum Depth** | {bathymetry.max():.1f} m |
                            | **Mean Depth** | {bathymetry.mean():.1f} m |
                            | **Deepest Point** | Row {np.unravel_index(bathymetry.argmax(), bathymetry.shape)[0]+1}, Col {np.unravel_index(bathymetry.argmax(), bathymetry.shape)[1]+1} |
                            """)
        ...
        with swan_sub2:
            ...
            if st.button("Calculate Monthly Maxima", key="calc_monthly"):
                with st.spinner("Calculating monthly maxima..."):
                    ...
                    if use_debiased and st.session_state.data_source != "Demo":
                        wind_speeds = wind_speeds * 0.718
                    df_copy['wave_height'], df_copy['wave_period'] = zip(*[calculate_wave_height(ws, fetch_km) for ws in wind_speeds])
...
#echo "✅ Updated era5_analysis.py with REAL bathymetry support"