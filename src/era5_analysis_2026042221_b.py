...
# Create directories
for d in [RAW_ERA5_DIR, DEBIASED_ERA5_DIR, CCMP_DIR, VALIDATION_DIR, WIND_FILES_DIR]:
...
def main():
    ...
    <div class="domain-card">
        ...
        <table style="width: 100%; color: white;">
            <tr>
                <td><b>Center Point:</b></td>
                <td>16.53°S, 28.83°E</td>
                <td><b>Domain Size:</b></td>
                <td>4.0 km × 3.5 km</td>
            </tr>
            <tr>
                <td><b>West Boundary:</b></td>
                <td>28.81113°E</td>
                <td><b>East Boundary:</b></td>
                <td>28.84887°E</td>
            </tr>
            <tr>
                <td><b>South Boundary:</b></td>
                <td>16.54577°S</td>
                <td><b>North Boundary:</b></td>
                <td>16.51423°S</td>
            </tr>
            <tr>
    ...
    # ========================================================================
    # TAB 1: Data Acquisition (WITH DEBIASING & VALIDATION BUTTONS)
    # ========================================================================
    with tab1:
...
c.retrieve(
    ...
    'era5_download.nc')
        ...
        # SUB-TAB 2: DEBIASING BUTTON (RESTORED)
        with sub2:
            ...
            # Single file debiasing
            st.divider()
            st.subheader("📄 Single File Debiasing")
            uploaded_file = st.file_uploader("Or upload a single NetCDF file", type=['nc', 'nc4'])
            if uploaded_file is not None:
                temp_path = os.path.join("/tmp", uploaded_file.name)
                with open(temp_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                
                if st.button("Debias This File"):
                    with st.spinner("Debiasing..."):
                        success, output = debias_era5_file(temp_path, DEBIASED_ERA5_DIR)
                        if success:
                            st.success(f"✅ Debiased file saved to: {output}")
                        else:
                            st.error("Debiasing failed")
        
        # SUB-TAB 3: VALIDATION BUTTON (RESTORED)
        with sub3:
            ...
            if st.button("Run Validation", type="primary", use_container_width=True):
                if not era5_files or not ccmp_files:
                    ...
                    with st.spinner("Validating against CCMP data..."):
                        results = []
                        for era5_file in era5_files[:5]:  # Limit to first 5 for speed
                ...
                if st.button("📊 Process Files", use_container_width=True, type="primary"):
                    if not XARRAY_AVAILABLE:
                        ...
                        with st.spinner("Processing..."):
                            result = process_all_files(use_dir)
                            if result:
