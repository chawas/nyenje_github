...
def main():
    ...
    with tab2:
        ...
        with wsub2:
            comp_df = pd.DataFrame({
                'Parameter': ['Hourly', '3-min', '10-min', '3-sec Gust', '10-sec Gust'],
                'Mean (m/s)': [
                    df['wind_speed_hourly'].mean(), df['wind_speed_3min'].mean(),
                    df['wind_speed_10min'].mean(), df['gust_3sec'].mean(), df['gust_10sec'].mean()
                ],
                'Max (m/s)': [
                    df['wind_speed_hourly'].max(), df['wind_speed_3min'].max(),
                    df['wind_speed_10min'].max(), df['gust_3sec'].max(), df['gust_10sec'].max()
                ]
            })
            st.dataframe(comp_df.round(2), use_container_width=True)
            fig = go.Figure()
            fig.add_trace(go.Bar(x=comp_df['Parameter'], y=comp_df['Mean (m/s)'], name='Mean', marker_color='steelblue'))
            fig.add_trace(go.Bar(x=comp_df['Parameter'], y=comp_df['Max (m/s)'], name='Max', marker_color='coral'))
            fig.update_layout(title="Wind Speed Comparison", barmode='group', height=400)
            st.plotly_chart(fig, use_container_width=True)
    ...
    # ========================================================================
    # TAB 4: SWAN Model (WITH ENLARGED WAVE HEIGHT MAP)
    # ========================================================================
    with tab4:
