...
# Make a backup
#cp era5_analysis.py era5_analysis.py.backup2

# Create the corrected file with bathymetry map
...
def main():
    ...
    with tab4:
        ...
        with swan_sub1:
            ...
            # Display REAL Bathymetry Map FIRST
            st.subheader("🗺️ Nyenje Bay Bathymetry")
            if REAL_BATHYMETRY is not None:
                ...
                fig_bathy.update_layout(
                    title="Nyenje Bay Bathymetry (REAL Data)",
            ...
            st.info(f"📊 Using REAL Nyenje Bay bathymetry (Depth range: {REAL_BATHYMETRY.min():.1f}m - {REAL_BATHYMETRY.max():.1f}m)" if REAL_BATHYMETRY is not None else "📊 Using synthetic bathymetry")
...
#echo "✅ Complete era5_analysis.py with bathymetry map restored"