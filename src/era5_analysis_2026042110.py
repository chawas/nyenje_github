...
# ============================================================================
# Wind File Extraction for Nyenje Bay
# ============================================================================
...
def extract_swan_wind_from_era5(nc_file, output_dir, debias_factor=0.718):
    """Extract wind for Nyenje Bay 9x8 grid from ERA5 file"""
...
# ============================================================================
# File Processing Functions for Statistics
# ============================================================================
...
def main():
    ...
    # ========================================================================
    # TAB 1: Data Acquisition & Processing (WITH ALL SUB-MENUS)
    # ========================================================================
    with tab1:
        ...
        sub1, sub2, sub3, sub4 = st.tabs([
            " ERA5 Download", "🛰️ CCMP Download", " Process Data", " SWAN Extract"
        ])
...
c.retrieve(
    'reanalysis-era5-single-levels',
    {
        ...
        'area': [10, 26, 18, 30],  # North, West, South, East
        'format': 'netcdf',
    },
    'era5_download.nc')
            """, language="python")
        ...
        with sub2:
            ...
            **Why CCMP?**
            ...
            st.info("💡 After downloading CCMP data, place files in `data/ccmp/` directory")
        
        # ================================================================
        # SUB-TAB 3: Process Data
        # ================================================================
        with sub3:
            ...
            data_dir = st.text_input(
                "Data Directory", 
                value=os.path.join(BASE_DIR, "data/era5/raw"),
                key="process_dir"
            )
        ...
        with sub4:
            ...
            st.caption(f"📁 Existing wind files: {len(existing)} .wnd files")
            
            if st.button("🚀 Extract SWAN Winds", key="extract_swan", use_container_width=True):
                if not XARRAY_AVAILABLE:
                    st.error("Install: pip install xarray netCDF4 cfgrib")
                    ...
                    with st.spinner(f"Extracting wind files for {swan_year}..."):
                        ...
                        if not year_files:
                            st.warning(f"No NetCDF files found for year {swan_year}")
                        else:
                            progress_bar = st.progress(0)
                            status_text = st.empty()
                            total_extracted = 0
                            ...
                            for idx, nc_file in enumerate(year_files):
                                ...
                                count, created = extract_swan_wind_from_era5(nc_file, swan_output_dir, debias_factor)
                            ...
                            if all_files:
                                st.subheader("Sample of created files:")
