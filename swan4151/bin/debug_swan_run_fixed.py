#!/usr/bin/env python3
"""
Debug SWAN run function - Fixed PROJECT string length
"""
...
def run_swan_for_timestamp(year, timestamp, wind_file_path):
    ...
    # Short project run number (max 4 characters as per SWAN limit)
    short_timestamp = timestamp.replace('_', '')[-8:]  # Take last 8 chars (HHMM from _HHMM)
    
    print(f"\nDebug info:")
    print(f"  Year: {year}")
    print(f"  Timestamp: {timestamp}")
    print(f"  Short run ID: {short_timestamp}")
    print(f"  Wind file exists: {os.path.exists(wind_file_path)}")
    
    # Copy wind file to archive
    if os.path.exists(wind_file_path):
        shutil.copy(wind_file_path, archived_wind)
        print(f"  Copied wind file to archive")
    else:
        print(f"  ERROR: Wind file not found!")
        return None, None
    
    # Create SWAN input file with shorter PROJECT string
    swan_content = f"""PROJECT 'ERA5' '{short_timestamp}' 'ERA5 run' ' ' ' '
...
"""
    ...
    # Create INPUT file for SWAN with shorter PROJECT string
    input_content = f"""PROJECT 'ERA5' '{short_timestamp}' 'ERA5 run' ' ' ' '
MODE STATIONARY TWODIMENSIONAL
COORDINATES CARTESIAN
CGRID REGULAR 0.0 0.0 0.0 4000.0 3500.0 8 7 CIRCLE 36 0.04 1.00 25
INPGRID BOTTOM REGULAR 0.0 0.0 0.0 8 7 500.0 500.0
...
print("=" * 60)
print("Testing SWAN Runner (Fixed PROJECT string)")
print("=" * 60)
...
if os.path.exists(year_dir):
    ...
    if os.path.exists(WIND_SOURCE_DIR):
        print(f"\nContents of {WIND_SOURCE_DIR}:")
        for item in os.listdir(WIND_SOURCE_DIR):
            print(f"  - {item}")
    else:
